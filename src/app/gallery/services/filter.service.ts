...
export class FilterService {
  ...
  public readonly filterFavorites: Filter = { state: 0 };
  public readonly filterBookmarks: Filter = { state: 0 };
  public readonly filterGroups: Filter = { state: 0 };
  ...
  private doesPassFilters(image: GalleryImage): boolean {
    ...
    if (!this.doesPassFilter(this.filterFavorites, image.heart)) {
      return false;
    }

    if (!this.doesPassFilter(this.filterBookmarks, image.bookmark)) {
      return false;
    }

    if (!this.doesPassFilter(this.filterGroups, image.group != null)) {
      return false;
    }
